...
def main():
    ...
    try:
        ...
        logging.error(f"HTTP 請求最終失敗: {e}", exc_info=False)
        ...
        if not alert_cfg["enabled"]:
            ...
            send_email("【Warning】地震速報程式錯誤通知", f"地震資料抓取失敗(重試後)，請檢查網路連線或 API 金鑰。\n\n錯誤詳情:\n{e}")
            send_line_message("【Warning】地震速報程式錯誤：地震資料抓取失敗(重試後)！")
        else:
            ...
            # 只有在達到門檻時，才定義為需要人工介入的「錯誤」
            if failures_count >= threshold:
                logging.error(f"【注意】連續 {error_name} 失敗已達 {threshold} 次門檻，觸發告警！")
                send_email(f"【Warning】地震速報程式連續 {threshold} 次連線失敗", f"地震資料抓取已連續發生 {threshold} 次 {error_name} 錯誤，請檢查伺服器網路狀態或 CWA API 服務。\n\n最新錯誤詳情:\n{e}")
                send_line_message(f"【Warning】地震速報程式連續 {threshold} 次連線失敗 ({error_name})！")
                system_status[counter_key] = 0
    finally:
        # ---------------------
        # 4. 清理舊檔案
        # ---------------------
